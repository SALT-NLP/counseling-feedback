...
with open("./validate_skills.json", "r") as f:
...
model_path = "./best_setfit_deberta_f1_0_5896"
...
for example in validation_data:
    ...
    if alternative_response.strip():  # only include alternative if it exists
...
# ensure labels are correctly shaped (num_samples, num_labels)
if labels.ndim == 1:
    labels = labels.reshape(-1, len(SKILL_CATEGORIES))

y_pred = model.predict(texts)

# check if y_pred is single-dimensional
if y_pred.ndim == 1 or y_pred.shape[1] != len(SKILL_CATEGORIES):
    ...
    # manually expand predictions if they are mistakenly single-output
